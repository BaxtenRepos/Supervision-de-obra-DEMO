...
var ProyectoItems, AvanceFisicoItems, AvanceFinancieroItems, ProyectosDirectivoItems, RepMaquinariaItems;
...
function init(){
 ...
 var i = 0;
 function lee_json(){
    $.getJSON("https://apis-explorer.appspot.com/apis-explorer/?base=https://1-dot-iuyet-csgit-cao.appspot.com/_ah/api#p/communicationchannel/v1/communicationchannel.listAvance_Fisico?id_referencia=1&Tipo_Entidad=0&_h=2&", function(datos){
        console.log("Dato: " + items["id_AvanceFisico"]);
        $.each(items["id_referencia"], function(){
            console.log("id de referencia: " + id_referencia);
        });
    });
}
//---------------------------------
function reporte_diario(idObra) {
           //code
...
function reporte_maquinaria(reporte_diario) {
           //code
           gapi.client.communicationchannel.listRepMaquinariaCatMaquinaria({"idReporteDiario":reporte_diario, "email":" "}).execute(function(resp){
                      if (resp.code) {
                                 ...
                                 /*for (var i = 0; i < ProyectosDirectivoItems.length; i++) {
                                            ...
                                            gapi.client.communicationchannel.getProyecto({"id": ProyectosDirectivoItems[i].id_proyecto}).execute(function(resp){
                                                       if (resp.code) {
                                                                  ...
                                                                  listaproyectositems = resp.items;
                                                                  console.log("id de Proyecto del get: " + listaproyectositems.length);
                                                       }        
                                            });
                                 }*/
                      }
...
function crearTablas(tx) {
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS MultimediaMaquinaria (idMultimedia unique, idMaquinaria, TipoArchivo, Path, Fecha, Descripcion)');
    //Programa de Obra
            ...
            TipoMaquinaria=MaquinariaItems[x].id_tipo_Maquinaria;
             ...
             db.transaction(function(tx) {
                 ...
                 tx.executeSql('INSERT INTO Maquinaria (idMaquinaria, nombre, descripcion, tipo)' +
                               'VALUES (?, ?, ?, ?)',
                                [idMaquinaria, NombreMaquinaria, DescripcionMaquinaria,TipoMaquinaria]);
...
function insertarMultimedia(x) {
           ...
           db.transaction(function(tx) {
                   ...
                   if (M_TipoEntidad == 2 || M_TipoEntidad == "2") {
                                console.log("Insertando multimedia Obras: " + idMultimedia + " idObra : "+M_idReferencia); 
                                tx.executeSql('INSERT INTO MultimediaMaquinaria(idMultimedia, idMaquinaria, TipoArchivo, Path, Fecha, Descripcion)' +
                               ' VALUES (?, ?, ?, ?, ?, ?)',
                                [idMultimedia, M_idReferencia, M_TipoArchivo, M_Path, M_Fecha, M_Descripcion]);
                   }
            }, errorCB, function (){
...
function insertarRepMaquinaria(x) {
           //code
           if (RepMaquinariaItems) {
                      console.log("cargando los datos a la base de datos.");
                      //code
                      if (x < RepMaquinariaItems.length ) {
                                 //code
                                 C_idreportmaq = RepMaquinariaItems[x].idRepMaquinaria;
                                 C_idreporte = RepMaquinariaItems[x].idReporteDiario;
                                 C_idcatmaquinaria = RepMaquinariaItems[x].idCatMaquinaria;
                                 C_email = RepMaquinariaItems[x].email;
                                 C_cantidad = RepMaquinariaItems[x].cantidad;
                      
                                 db.transaction(function(tx) {
                                            tx.executeSql('INSERT INTO RepMaquinaria(idReporteMaq, idReporte, idCatMaquinaria, email, cantidad)' +
                                                     'VALUES (?, ?, ?, ?, ?)',
                                                      [C_idreportmaq, C_idreporte, C_idcatmaquinaria, C_email, C_cantidad]);
                                            }, errorCB,function (){
                                                       console.log("Se inserto correctamente documentos de proyectos: ");
                                                       x++;
                                                       insertarRepMaquinaria(x);
                                            }
                                 );
                      }else{
                             console.log("No hay Reportes de maquinaria");
                      insertarTipoMaquinaria(0);    
                      }
           }
}
/*function insertarMaquinaria(x) {
           //code
           if (ListMaquinariaItems) {
                      console.log("cargando los datos a la base de datos.");
                      //code
                      if (x < ListMaquinariaItems.length ) {
                                 //code
                                 M_idmaquinaria = ListMaquinariaItems[x].id_Maquinaria;
                                 M_idtipomaquinaria = ListMaquinariaItems[x].id_tipo_Maquinaria;
                                 M_idnombre = ListMaquinariaItems[x].nombre;
                                 M_descripcion = ListMaquinariaItems[x].descripcion;
                      
                                 db.transaction(function(tx) {
                                            tx.executeSql('INSERT INTO Maquinaria (idMaquinaria, nombre, descripcion, tipo)' +
                                                     'VALUES (?, ?, ?, ?)',
                                                      [M_idmaquinaria, M_idtipomaquinaria, M_idnombre, M_descripcion]);
                                            }, errorCB,function (){
                                                       console.log("Se inserto correctamente documentos de proyectos: ");
                                                       x++;
                                                       insertarMaquinaria(x);
                             ...
                             console.log("No hay Lista de maquinaria");
                      insertarTipoMaquinaria(0);    
                      }
           }
}*/
function insertarTipoMaquinaria(x) {
           //code
