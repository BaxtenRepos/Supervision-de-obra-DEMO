...
function cargar2(){
 ...
 function fecha_2(fecha1, fecha2) {
	...
	tx.executeSql('SELECT Descripcion, Path, Fecha FROM MultimediaObras WHERE cast(idObra as int) = ? and Formato == "0" or  Formato == "5"',[obra],  function(tx, rs) {
	...
	tx.executeSql('select * from MultimediaObras where cast(idObra as int) = ?',[obra],  function(tx, rs) {
		...
		for (var i = 0; i < rs.rows.length; i++) {
		    ...
		    var desc = p.Descripcion;
			    document.getElementById("links").innerHTML =
			    "<center>" +
				"<a href=\"http://1-dot-cao-iuyet-server.appspot.com/serve?blob-key=" + p.Path + "\" class=\"disabled\" title=\"\" data-gallery=\"\" data-unique-id=\"" + p.idMultimedia + "\">" +
					"<img src=\"http://1-dot-cao-iuyet-server.appspot.com/serve?blob-key=" + p.Path + "\" /> " +
				"</a>" +
			    "</center>";
		    cargar2();   
