...
if (window.location.hostname == 'localhost'
    ...
    //ROOT = 'http://' + '1-dot-iuyet-csgit-cao.appspot.com' + '/_ah/api';//desarrollo
    ROOT = 'http://' + '1-dot-superobra-adquem.appspot.com' + '/_ah/api';//adquem
    ...
    gapi.client.load('communicationchannel', 'v1', function(){
        gapi.client.communicationchannel.listProyectoDelDirectivo({"directivo":directivo}).execute(function(resp){
            if (resp.code) {
                ...
                console.log("Proyectos: " + ProyectoDirectivo.length);
        ...
        gapi.client.communicationchannel.listRepoPersonalCatPersonal({"email": " ", "def": "0"}).execute(function(resp){
            if (resp.code) {
                console.log("Error de conexi—n, no se ha podido descargar el reporte Personal Catalogo Personal, volviendo a intentar. :::" + resp.message );
...
function crearTablas(tx) {
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS Directorio(idPersona, Nombre, ApPaterno, ApMaterno, RFCPersonal, Cargo, TituloProfesional, CedulaProfesional, Fotografia, Skype, Email, Telefono, Radio)');
    //Directorio Proyectos
    tx.executeSql('DROP TABLE IF EXISTS DirectorioProyecto');
    tx.executeSql('CREATE TABLE IF NOT EXISTS DirectorioProyecto(id_Directorio, id_Referencia, id_Persona)');
    //Directorio Obras
    tx.executeSql('DROP TABLE IF EXISTS DirectorioObra');
    tx.executeSql('CREATE TABLE IF NOT EXISTS DirectorioObra(id_Directorio, id_Referencia, id_Persona)');
    //Obras
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS AlertasObras(idAlertaObra,  idObra, LimiteDesvio, Tipo)');
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS Programa_Obra (idPrograma , idObra, TipoAvance, Fecha, PorcentajeAvance)');
    //Programa de Proyecto
    tx.executeSql('DROP TABLE IF EXISTS Programa_Proyecto');
    tx.executeSql('CREATE TABLE IF NOT EXISTS Programa_Proyecto(idPrograma , idProyecto, TipoAvance, Fecha, PorcentajeAvance)');
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS CatPersonal (idCatPersonal, Tipo_Persona, Descripcion)');
    
    //Lista Tipo de Maquinaria
    tx.executeSql('DROP TABLE IF EXISTS RepCatPersonal');
    tx.executeSql('CREATE TABLE IF NOT EXISTS RepCatPersonal (idPropietario, idReporte, idCatPersonal, Cantidad, id_dispositivo, email)');
    }
...
function obras(x, o) {
    //code
    if (ProyectoDirectivo[x].obras) {
        //code
        if (ProyectoDirectivo[x].obras.length > o) {
        //code
            if (ProyectoDirectivo[x].obras[o].obra.visible == true) {
                //code
                console.log("visible: " + ProyectoDirectivo[x].obras[o].obra.visible);
...
function insertarCatPersonal(x) {
    ...
    if (CatPersonalItems) {
        //console.log("cargando los datos a la base de datos.");
                      //code
        if (x < CatPersonalItems.length) {
                                 //code
            if (CatPersonalItems[x].visible == true) {
                //case
        ...
        if (x < DirectorioItems.length) {
            ...
            switch (DirectorioItems[x].tipoReferencia){
                case '0':
                    ...
                    personas_directorio_proyecto(x, 0);
                    break;
                case '1':
                    console.log("Directorio Obra:");
                    personas_directorio_obra(x, 0);
                    break;
                default:
                    break;
            };
        }else{
            //en caso de que ya no existan
...
function personas_directorio_obra(x, i) {
    if (DirectorioItems[x].arregloPersonasId) {
        //code
        if (i < DirectorioItems[x].arregloPersonasId.length) {
            //code
            console.log("id de persona: " + DirectorioItems[x].arregloPersonasId[i]);
            DidDirectorio = DirectorioItems[x].idDirectorio;
            DidReferencia = DirectorioItems[x].idReferencia;
            DidPersona = DirectorioItems[x].arregloPersonasId[i];
            db.transaction(function(tx) {
                tx.executeSql('INSERT INTO DirectorioObra(id_Directorio, id_Referencia, id_Persona)' +
                            'VALUES (?, ?, ?)',
                            [DidDirectorio, DidReferencia, DidPersona]);
                }, errorCB, function (){
                    console.log("Se inserto correctamente directorio: ");
                    i++;
                    personas_directorio_obra(x, i);
            });
        }else{
            //code
            x++;
...
function personas_directorio_proyecto(x, i) {
    if (DirectorioItems[x].arregloPersonasId) {
        //code
        if (i < DirectorioItems[x].arregloPersonasId.length) {
            //code
            ...
            DidPersona = DirectorioItems[x].arregloPersonasId[i];
            db.transaction(function(tx) {
                tx.executeSql('INSERT INTO DirectorioProyecto(id_Directorio, id_Referencia, id_Persona)' +
                            'VALUES (?, ?, ?)',
                            [DidDirectorio, DidReferencia, DidPersona]);
                }, errorCB, function (){
                    console.log("Se inserto correctamente directorio: ");
                    i++;
                    personas_directorio_proyecto(x, i);
            });
        }else{
            //code
            x++;
...
function insertarDirectorioProyectos(x) {
    ...
    if (PersonaItems) {
        //code
        if (x < PersonaItems.length) {
            //code
            if (PersonaItems[x].visible == true) {
                //code
                ...
                db.transaction(function(tx){
                    tx.executeSql('INSERT INTO Directorio(idPersona, Nombre, ApPaterno, ApMaterno, RFCPersonal, Cargo, TituloProfesional, CedulaProfesional, Fotografia, Skype, Email, Telefono, Radio)'+
                                'VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                                [D_idpersona, D_nombre, D_apPaterno, D_apMaterno, D_rfc, D_cargo, D_tProfesional, D_cProfesional, D_fotografia, D_skype, D_email, D_telefono, D_radio]);
                    }, errorCB, function(){
...
function insertarProgramaProyecto(x) {
           ...
           db.transaction(function(tx) {
                 ...
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [30, 1, 1, "30/01/2015", "16"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [31, 1, 1, "20/02/2015", "24"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [32, 1, 1, "08/03/2015", "28"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [33, 1, 1, "02/04/2015", "35"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [34, 1, 1, "28/04/2015", "39"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [35, 1, 1, "20/05/2015", "42"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [36, 1, 1, "15/06/2015", "47"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [37, 1, 1, "10/07/2015", "52"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [38, 1, 1, "30/08/2015", "56"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [39, 1, 1, "26/09/2015", "60"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [40, 1, 1, "18/10/2015", "64"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [41, 1, 1, "06/11/2015", "70"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [42, 1, 1, "30/11/2015", "74"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [43, 1, 1, "20/12/2015", "79"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [44, 1, 1, "10/01/2016", "83"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [45, 1, 1, "02/02/2016", "89"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [46, 1, 1, "22/02/2016", "94"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [47, 1, 1, "14/03/2016", "95"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [48, 1, 1, "02/03/2016", "96"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
                                [49, 1, 1, "24/03/2016", "99"]);
                 tx.executeSql('INSERT INTO Programa_Proyecto (idPrograma, idProyecto, TipoAvance, Fecha, PorcentajeAvance)' +
                               'VALUES (?, ?, ?, ?, ?)',
