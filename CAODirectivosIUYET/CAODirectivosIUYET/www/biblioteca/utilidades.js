...
function getIdDirectivo(){
  ...
  db = window.openDatabase("Database", "1.0", "PhoneGap Demo", 200000);
              ...
              tx.executeSql('SELECT * FROM Usuario',[],  function(tx, rs) {
                      ...
                      if (rs.rows.length>0) {
                        ...
                        //* console.log("Usuario en la bse de datos: " + rs.rows.length +" Usuario:"+rs.rows.item(0).idPropietario);
                         ...
                         RFC = rs.rows.item(0).RFC;
                         cargo = rs.rows.item(0).Cargo;
                         foto = rs.rows.item(0).Fotografia;
            ...
            '</a>';
                 ...
                 document.getElementById("imagen_perfil_2").innerHTML =
                 "<img width=\"69\" height=\"69\" src=\"https://"+ruta+"/serve?blob-key="+foto+"\" data-src=\"https://"+ruta+"/serve?blob-key="+foto+"\" data-src-retina=\"https://"+ruta+"/serve?blob-key="+foto+"\">";
                 
                 document.getElementById("imagen_perfil").innerHTML =
                 "<img width=\"35\" height=\"35\" src=\"https://"+ruta+"/serve?blob-key="+foto+"\" data-src=\"https://"+ruta+"/serve?blob-key="+foto+"\" data-src-retina=\"https://"+ruta+"/serve?blob-key="+foto+"\">";
                 ...
                 '<h4 class="semi-bold no-margin">'+Nombre+' '+Apellido+' '+ApellidoM+'</h4>'+
                                       ...
                                       // '<p><span class="semi-bold">Cargo:</span> Director</p>'+
                                        ...
                                        '<p><span class="semi-bold">RFC: </span> '+RFC+'</p>'+
                                        '<p><span class="semi-bold">Cargo: </span> '+cargo+'</p>';
                                        //'<br>'+
...
function changePassOK(){
     ...
     passclave = CryptoJS.MD5(secret + clave);
     passnueva = CryptoJS.MD5(secret + nueva);
     passconfirmar = CryptoJS.MD5(secret + confirmar);
     console.log("clave: " + passclave);
     console.log("passnueva: " + passnueva);
     console.log("passconfirmar: " + passconfirmar);
     ...
     if (clave.trim()=="" || nueva.trim() == "" || confirmar.trim()=="") {
          ...
          if (Pass==passclave) {
               //code
               console.log("la contrase–as es igual");
               actualizarEndPointUser(passnueva);//comentada
               /*document.getElementById("errorGeneral").innerHTML= "";
               document.getElementById("errorConfirmar").innerHTML= "";
               document.getElementById("errorClave").innerHTML= "La clave no es correcta";*/
          }else{
               console.log("no conciden las contrase–as");
...
function actualizarEndPointUser(passnueva){
     console.log("pass: " + passnueva);

     //Consultar EndPoint Usuario
           gapi.client.load('usuarioendpoint', 'v1', function(){ 
               gapi.client.usuarioendpoint.updateUsuario({"id_Propietario": idDirectivo, "id_Persona": idPersonaBD, "id_Tipo_Persona": "2", "usuario": UsuarioBD, "contrasena": passnueva, }).execute(function(resp){
                  if(resp.code && !resp) {
                                   ...
                                   "<td>"+
                                        ...
                                        //"<div class=\"user-details-wrapper active\" data-chat-user-pic=\"assets/img/profiles/d.jpg\" data-chat-user-pic-retina=\"assets/img/profiles/d2x.jpg\">"+
                                             "<div class=\"user-profile\">"+
                                                  "<img src=\"https://"+ruta+"/serve?blob-key="+p.Fotografia+"\" alt=\"\" data-src=\"https://"+ruta+"/serve?blob-key="+p.Fotografia+"\" data-src-retina=\"assets/img/profiles/d2x.jpg\" width=\"35\" height=\"35\">"+
                                             "</div>"+
          ...
          tx.executeSql('select * from Directorio where cast(idPersona as int) = ?', [directorio1], function (tx, rs) {
               for(var i = 0; i<rs.rows.length;i++){
                    ...
                    document.getElementById("divPopupContent").innerHTML = 
                         "<div class=\"grid simple\">"+
                              "<div class=\"grid-title no-border\">"+
                                   "<img src=\"https://"+ruta+"/serve?blob-key="+p.Fotografia+"\" alt=\"\" data-src=\"https://"+ruta+"/serve?blob-key="+p.Fotografia+"\" data-src-retina=\"assets/img/profiles/avatar_small2x.jpg\" width=\"35\" height=\"35\" />"+
